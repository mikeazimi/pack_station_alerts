...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log(
      `[${new Date().toISOString()}] API /inventory/query - Querying with prefix: ${trimmedPrefix}`
    )
    ...
    console.log(
      `[${new Date().toISOString()}] API /inventory/query - ` +
    ...
    console.error(
      `[${new Date().toISOString()}] API /inventory/query - Error:`,
      errorMessage
    )
